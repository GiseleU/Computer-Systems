...
const int PAGE_SIZE = 16; // 4 KB = 2^12 bytes
const int PHYSICAL_MEMORY_SIZE = 64; // 32 KB = 2^15 bytes 
const int VIRTUAL_MEMORY_SIZE = 128; // 1 MB = 2^20 bytes
...
physical_addr translate_addr(virtual_addr va){
  ...
  int offset = va - start;
  page_table_entry_t entry = *(page_table + page_num);

  if (entry.valid){
    physical_addr pa = (entry.frame_number * PAGE_SIZE) + offset;
    return pa;
  }
