...
// global constants 16, 64, 128
// Note: I recommend testing with 
...
void initialize(int * data, int n){
  ...
  for(page_num = 0; page_num < NUM_FRAMES; page_num++){
    ...
    if(page_num < num_pages_n){
      ...
      entry_ptr->valid = 1;
      //global counter initiation here?
      // entry_ptr -> lru_counter +=1;
      entry_ptr->lru_counter = global_counter; 
...
physical_addr translate_addr(virtual_addr va){
  ...
  int page_num = va / PAGE_SIZE; 
  int start = page_num * PAGE_SIZE;
  int offset = va - start;
  // page_table_entry_t * entry_ptr = page_table + page_num;

  if (page_table[page_num].valid){
    physical_addr pa = (page_table[page_num].frame_number * PAGE_SIZE) + offset; 
    //update counter - if entry is used, global counter increments and lru counter for that entry is set
    global_counter++; 
    page_table[page_num].lru_counter = global_counter; 
    // if (va == 64){
      // printf("%d %d %d %d" , page_num, page_table[page_num].frame_number, pa, offset);
      // fflush(stdout);
    // }
    return pa;
  }
...
void handle_page_fault(virtual_addr va){
  ...
  if(strategy == RANDOM){
    ...
    for (i = 0; i < NUM_PAGES; i++) { 
      // page_table_entry_t * current_entry_ptr = page_table + i; 
      if (page_table[i].valid) { 
        // printf("%d %d \n", page_table[i].lru_counter, min);
        // If current page is valid, check its lru counter
        if (page_table[i].lru_counter < min) { 
          min = page_table[i].lru_counter; 
          evicted_page = i;
          evicted_entry_ptr = &page_table[i];  
        
        } 
      
      } 

    // printf("%d", evicted_page);
  ...
  // printf ("%d %d", evicted_page, NUM_PAGES);
  // fflush(stdout);
